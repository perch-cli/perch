...
fn grouping(host: &dyn Host, out: &mut dyn Write) -> Result<Step> {
    // Asked by the Alias where there is one: two Accounts may share an email
    // across providers, and the Alias is the Target that names one of them.
    let registry = adopt::ensure_adopted(host)?;
    let held: Vec<(String, Option<String>)> = registry
        .accounts
        .iter()
        .map(|account| {
            let target = registry.alias_of(account.key()).unwrap_or(account.email());
            (target.to_string(), account.group.clone())
        })
        .collect();
