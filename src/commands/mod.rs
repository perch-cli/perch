...
/// run `perch config set ungrouped interchangeable true` was still told Cycling
/// moves between these "when you say it may" — implying they had yet to say it.
///
/// A function rather than a third spelling. There were three: the bare
/// constant, "is on/off", and "is true/false" — and `on`/`off` is not a value
/// the Setting takes, so a reader who typed back what they were shown was
