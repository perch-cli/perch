...
/// and takes no option for another one (ADR one-thing-renders-the-site).
fn guide() -> PathBuf {
...
/// that was the guide's own index is now the splash page's card grid, asserted
/// by `the_landing_page_leads_into_the_guide` below
/// (ADR one-thing-renders-the-site).
///
...
/// (ADR one-thing-renders-the-site), so this is asserted in both directions:
/// every page it names is one the guide has, and every page the guide has is
/// one it names. The links are written as the paths the site serves —
/// `/perch/accounts/` — which is a spelling nothing else in the repository uses
...
/// (ADR one-thing-renders-the-site). A page without one does not build; a page
/// with one *and* an `#` heading of its own says its title twice, which builds
/// and is wrong.
#[test]
