...
/// and takes no option for another one.
fn guide() -> PathBuf {
...
/// Asserts that one link out of `from` lands on a page that exists and, where
/// it names one, a heading that exists. A URL is somebody else's to keep
/// working; `#section` on its own is a link into `from` and is checked.
...
/// `README.md` is the one index of the guide that is not generated: its links
/// are markdown paths rather than site URLs, so a page it stops naming is one an
/// offline clone cannot find. That it *mentions* every page, rather than that any
/// list within it does — it names most twice, and requiring both would assert
/// the shape of the README.
#[test]
...
/// browser against the site — and only the ones that stay inside the guide mean
/// the same thing to both, so a link up out of it is written as a URL.
#[test]
...
/// there is the URL already pasted into terminals. Neither the guide moving nor
/// the renderer changing may move it.
#[test]
...
/// Both halves matter: the first catches an installer that stopped being
/// mentioned, the second one that grew a version or moved under a prefix — a
/// command somebody has already pasted into a shell, now fetching nothing. The
/// set below is exhaustive by construction.
#[test]
...
/// Asserted in both directions, because the splash page is the guide's index as
/// well as the site's front page. The links are written as the paths the site
/// serves — `/perch/accounts/` — a spelling nothing else in the repository uses.
#[test]
...
///
/// Read line by line rather than by splitting on `"---\n"`, for the reason every
/// other helper here uses `lines()`: git hands a Windows checkout CRLF.
...
/// A page without one does not build; a page with one *and* an `#` heading of
/// its own says its title twice, which builds and is wrong.
#[test]
...
/// The other half of the same rule. This asks for the frontmatter too rather
/// than falling back to the whole document.
#[test]
