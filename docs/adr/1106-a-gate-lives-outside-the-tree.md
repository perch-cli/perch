...
token, so the blast radius is a runner that is destroyed afterwards. It is not
nothing, and it is not credentials.
...
review feedback waits for a click each time. That is the price of the middle
option and there is no setting that lowers it. It buys the guarantee that an
account created this morning cannot reach the runner that holds a keychain.
