...
> **Carried out in #173.** Like ADR using-it-is-the-proof,
> ADR a-watcher-knob-is-arithmetic, ADR a-command-names-its-noun,
> ADR a-switch-is-written-down-first, ADR the-listing-owns-the-set and
...
> of beside it. The tree now matches it: `Global`, `Override` and `Inherit` are
> gone, `registry::Scope` and `cycle::Scope` are one type, `perch config unset`
> is deleted, `cycle-ungrouped` is `interchangeable` and carried by the
> Ungrouped Scope alone, and the registry's `version` is `2`.

ADR a-watcher-knob-is-arithmetic left this behind as a count: five nouns —
Group, Scope, Override, Ungrouped, Global — serving three Settings, one of which
uses none of them. The count is wrong in both directions, and correcting it is
most of the decision.

`CONTEXT.md`'s Configuration section holds **seven** entries: Setting, Config,
Scope, Global, Ungrouped, Override, **Inherit**. The list of five names
**Group**, which is not in that section and is not configuration's to delete — a
Group is the Cycling boundary (ADR a-group-is-a-declaration), and it exists
whether or not it carries a value. It names **Ungrouped**, which `cycle::Scope`
and `cycle::may_cycle_within` need whatever happens here. And it omits
**Inherit**, which is machinery and nothing else.
...
ADR a-watcher-knob-is-arithmetic put the best remaining argument for Overrides
on the record and handed it here as evidence: *a work Group wanting a different
threshold from a personal one.*
...
#151 and ADR a-suite-is-named-and-gated were. But nothing is being taken away from anybody.

## The asymmetry the count leaned on does not exist

ADR a-group-is-a-declaration's amendment, `commands/watch.rs:523`,
`commands/config.rs:869` and `docs/guide/configuration.md:86` all state the same
rule: `watcher-may-act` does not reach the Accounts in no Group by Inheritance,
so the watcher acts there only on two independent yeses.
...
including the one whose doc comment calls itself "ADR a-group-is-a-declaration,
amended, in executable form" and "the case most likely to be 'fixed' by somebody
tidying the layering into uniformity, which is why it names the ADR". It named
the ADR and still did not assert it.
...
Global within reach of the ranking, which is the one thing
ADR a-group-is-a-declaration is about." That hazard cannot occur once there is
no `Global` variant, so the two become one type and the warning becomes a
sentence about a mistake nobody can make.
...
consent, and layering consent is what produced the hole above. It also produces
a quieter one that has never been examined: a Global `watcher-may-act true`
...
about. ADR a-group-is-a-declaration worried about exactly that shape for
ungrouped Accounts and accepted it there deliberately; for Groups it arrived
unexamined, by inheritance.

Saying a grant only about the Scope it grants makes
ADR a-group-is-a-declaration's two yeses structural. The fourth row of the table
above is not fixed — it cannot be written down.

**The cost is real and is accepted: there is no longer one command that
withdraws the watcher everywhere.** ADR the-machine-runs-the-watcher's held
Service is paused by withdrawing the grant, and that becomes one command per
Scope. The grant was never the only brake — the loop is a foreground process you
kill and a Service is one you stop — and a brake that works by blanket
inheritance is the wrong brake for consent.
...
Scope of the Account you are on — the shallow end restored, without Global.
ADR a-command-names-its-noun's elision works because the unsaid noun is *always
the same one*; here the unsaid subject changes with whichever Account happens to
be active, so the identical command means different things on different days.
Tolerable for a Switch, which says what it did. Not tolerable for a rule that
persists after the sentence has scrolled away.
...
The name repeating its own Scope is the defect ADR a-command-names-its-noun
found in commands, and the same answer applies. `may-cycle` was the runner-up —
it pairs with `watcher-may-act` and matches `cycle::may_cycle_within`, the
predicate it already is — and was refused because it recasts a declaration as a
grant. ADR a-group-is-a-declaration is careful that this is *a person declaring
their Accounts substitutable*, not *a permission handed to Perch*, and that
distinction is the whole reason the two yeses are two rather than one thing said
twice. Two grants side by side would read as redundant. A declaration and a
grant do not.
...
Settings — a Renamed Group still brings everything with it, and declaring one
and forgetting another still loses the lot. **Setting**, **Config** and
**Group** are untouched; ADR a-group-is-a-declaration's "a Group carries the
settings that govern when it may Cycle unasked" is more literally true after
this than before it.
...
**ADR a-group-is-a-declaration's amendment, "Global carries the defaults and a
Group Overrides them" — superseded in full.** Every sentence in it is about
Global, Override, Inherit, the two layers or the word-count idiom.
**ADR a-group-is-a-declaration's body is untouched**, and better than untouched:
"A group also carries configuration: whether the watcher may switch accounts
within it unattended, at what utilization threshold, and which strategy it
prefers" names precisely the three Settings a Group now holds outright, which is
a sentence the amendment made approximate and this restores word for word. The
second leg Groups stand on is strengthened rather than cut.

**ADR a-group-is-a-declaration's amendment, "the Ungrouped Accounts are a Scope
that reads Global" — superseded in full.** Its core claim survives and is
strengthened: they are a Scope, and now hold Settings outright rather than
Overrides over something else. What goes is "reads Global" and the whole
non-uniform-layering paragraph, which describes an exception that can no longer
be expressed.

**ADR a-group-is-a-declaration's body loses one sentence.** "The setting is
global rather than per-Group" is now false. The two sentences of reasoning under
it survive intact — an ungrouped Account has no Group to carry the declaration,
and modeling the ungrouped pool as a Group with a reserved name would make a
Group mean two contradictory things — and both are still why the Ungrouped
Accounts are a Scope and never a Group. The Consequences section's "the shape of
that form is for the configuration spec to settle, along with the key's name" is
settled here.

Superseding ADR a-group-is-a-declaration whole was considered and refused, on
ADR a-watcher-knob-is-arithmetic's precedent: supersede the section that decayed
and leave a mostly-correct record standing rather than restating it at length.
0017's record is longer and cleaner than 0013's was, and superseding it entire
to correct one sentence whose *reasoning* survives would bury three Considered
Options that are the only written record of why ungrouped Accounts do not Cycle
freely.

**ADR a-watcher-knob-is-arithmetic is answered, not superseded.** This is the
question it produced and stopped at. One clause of its "What this does not
decide" decays — `watcher-threshold-percent` is still per-Scope, but no longer
"Overridable per Scope exactly as it is" — and its decision, which knobs
survive, is untouched.

**ADR a-command-names-its-noun and ADR the-machine-runs-the-watcher are
unamended.** `perch config unset` leaving is a consequence for the carry-out
rather than a revision of 0047, which this obeys; and 0040's held Service still
holds on a grant it still reads, at a Scope it already asks about.
...
**Sequenced after #161**, which removes ADR a-watcher-knob-is-arithmetic's three
departing Settings and is itself blocked on #169 removing the TUI. Converting
six Settings' worth of Override machinery that three of are about to be deleted
is work done twice, and `tui/model.rs` and `tests/browsing.rs` both reach it.
