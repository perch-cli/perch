...
Supersedes the part of ADR a-watcher-knob-is-arithmetic that rejected a managed
background daemon. The rest of that record stands — the interval, the back-off
curve, the cooldown, the margin, the no-return, the exit-code table and the
reasoning behind every one of them are unchanged and are still where they were
written. What changes is that `perch watch` is no longer only something you
type.
...
ADR a-watcher-knob-is-arithmetic gave two reasons for refusing a daemon. Only
one of them was ever a reason.

The first was safety: a daemon "would mutate credentials while nobody is
watching — every hazard in ADR a-switch-is-written-down-first firing
unattended". That argument was spent the moment the same record blessed
`perch watch --once` under cron. A Check scheduled at 02:00 mutates credentials
with nobody watching, by exactly the same mechanism, with exactly the same
hazards. Whatever unattended switching costs, Perch has been charging it since
ADR a-watcher-knob-is-arithmetic shipped; a Service does not add a risk, it adds
a way of arranging one that was already permitted.
...
agrees with it harder than ADR a-watcher-knob-is-arithmetic did. Perch does not
fork, does not double-fork, writes no PID file, and never puts itself in the
background. The operating system starts the process and the operating system
stops it. Perch's contribution is a unit file and the honesty to remove it
again.
...
minutes when a Refresh cannot be read, and the first Refresh that works drops
the whole of it. A Check has no memory to keep it in, so a sequence of Checks
has no Back-off at all — a fixed-interval timer asks a refusing endpoint
twenty-four times an hour, for ever, which is precisely the arithmetic
ADR a-watcher-knob-is-arithmetic wrote the Back-off to prevent. Persisting it
would work, and would be wrong twice over: it would put Anthropic's allowance in
the registry beside the Group's settings, as if it were one of them, and it
would give the loop a file to write when the whole of
ADR a-watcher-knob-is-arithmetic's cooldown reasoning is that a loop's pacing
belongs to the loop.
...
anyone's preference — ADR a-watcher-knob-is-arithmetic is explicit that it is a
constant and not a setting — so "poll less when idle" is not a knob this design
has to offer, and inventing one would mean inventing a definition of idle.
...
would also have no keychain: ADR claude-code-chooses-the-store has Perch
shelling out to `security`, and there is no unlocked keychain before login. Off
macOS the credential is a plaintext file (ADR claude-code-chooses-the-store), so
the keychain argument is macOS's alone — but the home directory argument is
every platform's, and it is sufficient.
...
terminals. ADR a-watcher-knob-is-arithmetic contemplated it — "two people
running two loops still pace themselves separately" — but that is about two
*people*, with two homes and two registries. Two loops for one person watch the
same active Account and each keeps its own Cooldown in memory, where neither can
see the other's. That is the ping-pong the Cooldown and the Margin exist to
prevent, reintroduced by running the thing twice.
...
produces for a contended lock and which ADR a-refusal-is-a-promise already
defines as a promise that nothing was changed. It takes the same lock as the
loop because the in-memory Cooldown and the one in `registry.checks` cannot see
each other — a Check firing while the Service runs is the same double-switch by
a different route. Somebody running both a cron Check and a Service gets a
mailbox of 20s, which is true and is the right nudge to pick one.
...
ADR a-watcher-knob-is-arithmetic has the loop stop when the grant is taken back:
`14` when a Group withdraws `watcher-may-act`, `18` when a `perch switch`
elsewhere leaves an ungrouped Account active. The reasoning was that "a loop
still running on permission that has been withdrawn is the exact thing 'nothing
changes underneath you unless you said it could' is about".
...
does not rank, it does not Switch. ADR a-watcher-knob-is-arithmetic conflated
stopping *acting* with stopping *existing*, which was correct when the only
Watcher was a loop in front of a person — a terminal loop idling for ever tells
that person nothing — and is incorrect for one nobody is sitting in front of. A
Service that exits because you have not granted permission yet is a Service you
have to remember to start again after you do.
...
at the first, exactly as ADR a-watcher-knob-is-arithmetic required, and it
re-checks on the ordinary interval rather than a faster one of its own: granting
permission and waiting up to two and a half minutes is not a cost worth a second
cadence to explain.
...
ADR a-watcher-knob-is-arithmetic required that "every held round says which
failure held it and when the watcher will ask again", and that a hold saying
neither "reads as a watcher that has given up". That was written about a person
watching a terminal, where the repeated line is the proof of life.
...
ADR a-watcher-knob-is-arithmetic refused a rotated logfile on the grounds that
"a rotated logfile is what a daemon needs because nobody is watching, and the
whole of this record is that Perch is not one". Perch is one now, and the
refusal survives anyway, because the unit file can route standard output better
than Perch can.
...
binary is still there. `perch upgrade` moves it
(ADR an-upgrade-asks-its-channel), so this is a condition that arises in
ordinary use rather than only under misuse.
...
success there worth having.

**An Upgrade re-installs the Service afterwards.**
ADR an-upgrade-asks-its-channel hands the work to `brew` or `npm`, neither of
which knows a unit exists; on Unix the running process keeps its inode and goes
on executing the old binary until something restarts it. So `perch upgrade`
rewrites the unit with the freshly resolved path and restarts it. A failure
there is a warning rather than a failed Upgrade: the binary really is newer, and
`perch service install` is idempotent precisely so that the fix is one command.
...
one Attended phase, because logging out and back in again is an act only a
person can perform (ADR using-it-is-the-proof).

> **Amended by ADR using-it-is-the-proof.** Both phases went with the Dogfood
> suite (#148), so nothing automated proves the three arrangements now — the
> paragraph above records what proved them, not what does. Nothing else here is
> repealed: the decision, the exit codes and the three arrangements all stand.
