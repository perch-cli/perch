# The existing login is adopted as the first profile
...
This falls out of profiles being real config directories (ADR 0001), and it is
worth protecting. A design that only ever snapshots the live credential has just
one place to log in, so adding an account means first logging out of the one you
are using — losing your session to gain an account. Perch never requires that.
