...
> **Superseded by ADR perch-does-not-draw.** Both halves of the decision below
> are void: `perch tui` is removed entire, so the tab this document repealed
> goes with the view it preserved, and the Config tab's removal never had to
> happen on its own. The reasoning is not what was found wrong — the machinery
> really is what the panel cost, and reversibility really was the wrong axis. It
> simply did not go far enough: the frame loop underneath both tabs is the same
> kind of cost, and nothing was found that wanted what it bought. One sentence
> here outlives the panel and is carried forward by 0049 rather than buried with
> it — "a surface which writes at all pulls in locking, deferral, refusal and
> rollback, and a surface that only reads and acts does not".
> ADR perch-does-not-draw stays superseded by this document; a chain is fine,
> and the route is worth as much as the destination.

ADR perch-does-not-draw moved a line. ADR perch-does-not-draw had said
`perch tui` acts on exactly two things, a Switch and a Run, because "a keystroke
away from an irreversible act is the wrong ergonomics for the one surface being
navigated by arrow key" — and 0034 observed, correctly, that a Setting is not
irreversible. Nothing one can be set to destroys anything. So the line moved to
where that reasoning pointed: the TUI may write what it can unwrite.

The observation was right and the conclusion is repealed. The Config tab and
every mechanism ADR perch-does-not-draw built to make its writes safe are to be
removed, and `perch tui` goes back to the two acts ADR perch-does-not-draw gave
it.
...
That is two subsystems sharing a frame loop. The Status tab is
ADR perch-does-not-draw's job — accounts ranked by headroom, group by group,
chosen between by eye. The Config tab is ADR perch-does-not-draw's — settings
per Scope, with `Inherit`, a debounce, a lock taken and given back per edit, a
refusal that rolls a row back, and the only place in Perch that accepts typed
input. They share no data, no verbs and no failure modes.
...
The panel's weight is not the settings it shows. It is everything
ADR perch-does-not-draw had to build so that showing them could also mean
changing them.
...
It is refused, and this time the reason is not the one ADR using-it-is-the-proof
gave.
...
## What ADR perch-does-not-draw keeps

One thing, and it is kept on merits rather than by inheritance.

`perch config unset` was grown by ADR perch-does-not-draw so that "the panel
cannot reach a state a script cannot" — clearing an Override back to Inherit.
With no panel, that justification is gone. The verb stays anyway, in both the
two-word and three-word forms.
...
This is worth naming for its own sake: the constraint ADR perch-does-not-draw
imposed — that every interactive capability exist non-interactively too —
improved the CLI in the one instance where it bit. That is a point in the
constraint's favor, and it survives the panel it was applied to.

## ADR perch-does-not-draw is re-affirmed, not rewritten
...
keeping. ADR perch-does-not-draw made a sound argument — a Setting really is not
irreversible — and still reached the wrong place, because it was measuring
recoverability when the cost was in the machinery. Editing that out of 0011
would erase the fact that the line was tested. The route is worth as much as the
destination.
...
ADR perch-does-not-draw noted that the TUI was the second thing that could lose
a lock mid-act; it stops being that.
