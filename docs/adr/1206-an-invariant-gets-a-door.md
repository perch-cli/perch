...
| a Watcher acts only while it holds the watch and has not been asked to stop | the Claude adapter's `service::send`, the only path from that module to the network, whose ask is a required parameter, and the `may_continue` every `DefaultInspection::resolve` takes, the only path to the Credential reads on the walk that settles a Landing; the port's `Network::http` is a `disallowed_methods` entry in `clippy.toml` naming the three senders that reach it, and `act::Watch::goes_on` is what a Watcher answers both asks with |
| a name rule holds the shape this build reads | `name::CURRENT_RULES`, one row of `name::Rule` variants and the fold they are told apart by; a rule is a variant, so one joining the build is one every `match` over the enum reports |
...
sites are correct and not which steps ask. It is a parameter of the Claude
adapter's `service::send` now — the one function every Anthropic request passes
through — and a sender that does not take one does not compile. Each move was
towards the narrowest place the invariant could be stated, and the two reviews
in between are what stating it anywhere wider cost. It has two doors rather than
one, because a round spends two things and only one of them is a request.
...
What the third door still does not reach is adoption and one local version read.
A seventh review left the whole of this open on the ground that `FakeHost` could
not stall a keychain read — which was already untrue when it was written, the
read-side stall having landed a day earlier, and which is what both tests here
are built on. The window stays, measured, and it is one process spawn wide.
...
**The ask at the keychain read.** The Claude adapter's `credentials::read` is
the one function every Credential Store read passes through, which is
`service::send`'s shape one module over, and it would reach every keychain walk
rather than this one. Refused on three counts. None of its callers is a command
— they are that adapter's own probe, backup, Profile and Default paths — so the
ask does not stop there: it is threaded up through everything that probes a
Profile, gathers an Export or Captures a Credential, and every command behind
those ends up holding one that means *nobody is watching*. It removes none of
the work above either — the walk still has to carry a stop out past the answer
that means *nothing says whose*, which is the decision this document is about.
And it states the invariant where there is nothing unbounded to stop: one
keychain read is one dialog, and what has no bound is the walk that makes one
per Account.
...
- A new request to Anthropic asks before it goes out, because a function in the
  Claude adapter's `service` that does not take the ask cannot reach the port. A
  new step that spends time *without* sending a request still asks by hand, and
  there this is a door rather than a wall: the alternative, a step protocol,
  stays refused in ADR an-ordering-is-a-type.
...
- `switch` names `watch::Lost` and `watch::StillOurs` while `watch` names
  ...
  Switch — so neither module is under the other. The Claude adapter's `service`
  already imports the same two.
